...
static void huffman_encode_init_from_codelist(HUFCODEC *phc)
{
    ...
    while (head < n - 2) /* 在一個 while 循環中計算碼長 */
    {
#if ENABLE_DEBUG_DUMP
        ...
        /* 根據 templist 中 head 開始的兩個元素的分組情況
           來更新 templist 中 head 之前相應元素的碼長和分組 */
        for (i=0; i<head; i++)
        {
            if (  templist[i].group == templist[head + 0].group
               || templist[i].group == templist[head + 1].group)
            {
                templist[i].depth++;       /* 碼長值加 1 */
                templist[i].group = group; /* 進行新的分組 */
            }
        }
...
void huffman_decode_init(HUFCODEC *phc)
{
    ...
    /* 根據哈夫曼表構造 first 表和 index 表
       first[i] 表示長度為 i+1 的第一個碼字的值
       index[i] 表示長度為 i+1 的第一個碼字的索引 */
    phc->first[0] = 0 ;
